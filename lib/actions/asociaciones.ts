...
export async function getAsociaciones() {
  try {
    ...
    const { data, error } = await supabase
      .from("asociaciones")
      .select("*, providers:providers(count)")
...
export async function getAsociacion(id: string) {
  const supabase = await createServiceRoleClient()
  const { data, error } = await supabase.from("asociaciones").select("*, providers:providers(*)").eq("id", id).single()
...
export async function updateAsociacion(id: string, formData: FormData) {
  ...
  const supabase = await createServiceRoleClient()
  const { error } = await supabase.from("asociaciones").update({ code, name, description }).eq("id", id)
