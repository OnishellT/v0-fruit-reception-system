-- Migration 34: Fix trigger to remove ORDER BY (no timestamp column in lab_samples)
-- Date: 2025-11-01
-- Purpose: Remove ORDER BY created_at from trigger since column doesn't exist
...
BEGIN
  ...
  -- Get lab sample quality metrics (using any lab sample with results)
...
DO $$
BEGIN
  RAISE NOTICE 'Migration 34 completed - trigger fixed (removed ORDER BY)';
END $$;
