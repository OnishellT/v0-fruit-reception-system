-- Migration 33: Add quality discounts from lab samples
-- Date: 2025-11-01
-- Purpose: Update trigger to also apply quality discounts based on lab sample quality metrics
...
DECLARE
  ...
  -- Reception-level quality metric values
  v_violetas_val DECIMAL(5,2);
  v_humedad_val DECIMAL(5,2);
  v_moho_val DECIMAL(5,2);

  -- Lab sample quality metric values
  v_lab_violetas_val DECIMAL(5,2);
  v_lab_moho_val DECIMAL(5,2);
  v_lab_basura_val DECIMAL(5,2);

  -- Individual discount percentages for reception-level quality
  v_violetas_discount_pct DECIMAL(5,2) := 0;
  v_humedad_discount_pct DECIMAL(5,2) := 0;
  v_moho_discount_pct DECIMAL(5,2) := 0;

  -- Individual discount percentages for lab sample quality
  v_lab_violetas_discount_pct DECIMAL(5,2) := 0;
  v_lab_moho_discount_pct DECIMAL(5,2) := 0;
  v_lab_basura_discount_pct DECIMAL(5,2) := 0;

  -- Individual discount amounts for reception-level quality
  v_violetas_discount DECIMAL(10,2) := 0;
  v_humedad_discount DECIMAL(10,2) := 0;
  v_moho_discount DECIMAL(10,2) := 0;

  -- Individual discount amounts for lab sample quality
  v_lab_violetas_discount DECIMAL(10,2) := 0;
  v_lab_moho_discount DECIMAL(10,2) := 0;
  v_lab_basura_discount DECIMAL(10,2) := 0;
...
BEGIN
  ...
  -- Get reception-level quality evaluation data
  ...
  -- Get lab sample quality metrics (using the latest lab sample)
  SELECT violetas_percentage, moho_percentage, basura_percentage
  INTO v_lab_violetas_val, v_lab_moho_val, v_lab_basura_val
  FROM laboratory_samples
  WHERE reception_id = p_recepcion_id
    AND dried_sample_kg IS NOT NULL
  ORDER BY created_at DESC
  LIMIT 1;

  -- Get reception-level thresholds for each metric separately
  ...
  -- Get lab sample thresholds for each metric separately
  -- Violetas (lab sample)
  SELECT dt.discount_percentage INTO v_lab_violetas_discount_pct
  FROM discount_thresholds dt
  JOIN pricing_rules pr ON dt.pricing_rule_id = pr.id
  WHERE dt.quality_metric = 'Violetas'
    AND pr.fruit_type = (SELECT ft.type FROM fruit_types ft WHERE ft.id = (SELECT fruit_type_id FROM receptions WHERE id = p_recepcion_id))
    AND v_lab_violetas_val > dt.min_value
    AND v_lab_violetas_val <= dt.max_value
  ORDER BY dt.discount_percentage DESC
  LIMIT 1;

  -- Moho (lab sample)
  SELECT dt.discount_percentage INTO v_lab_moho_discount_pct
  FROM discount_thresholds dt
  JOIN pricing_rules pr ON dt.pricing_rule_id = pr.id
  WHERE dt.quality_metric = 'Moho'
    AND pr.fruit_type = (SELECT ft.type FROM fruit_types ft WHERE ft.id = (SELECT fruit_type_id FROM receptions WHERE id = p_recepcion_id))
    AND v_lab_moho_val > dt.min_value
    AND v_lab_moho_val <= dt.max_value
  ORDER BY dt.discount_percentage DESC
  LIMIT 1;

  -- Basura (lab sample)
  SELECT dt.discount_percentage INTO v_lab_basura_discount_pct
  FROM discount_thresholds dt
  JOIN pricing_rules pr ON dt.pricing_rule_id = pr.id
  WHERE dt.quality_metric = 'Basura'
    AND pr.fruit_type = (SELECT ft.type FROM fruit_types ft WHERE ft.id = (SELECT fruit_type_id FROM receptions WHERE id = p_recepcion_id))
    AND v_lab_basura_val > dt.min_value
    AND v_lab_basura_val <= dt.max_value
  ORDER BY dt.discount_percentage DESC
  LIMIT 1;

  -- Calculate reception-level discounts
  ...
  -- Calculate lab sample quality discounts
  IF v_lab_violetas_val IS NOT NULL AND v_lab_violetas_discount_pct > 0 THEN
    v_lab_violetas_discount := v_original_weight * v_lab_violetas_discount_pct / 100;
  ELSE
    v_lab_violetas_discount := 0;
  END IF;

  IF v_lab_moho_val IS NOT NULL AND v_lab_moho_discount_pct > 0 THEN
    v_lab_moho_discount := v_original_weight * v_lab_moho_discount_pct / 100;
  ELSE
    v_lab_moho_discount := 0;
  END IF;

  IF v_lab_basura_val IS NOT NULL AND v_lab_basura_discount_pct > 0 THEN
    v_lab_basura_discount := v_original_weight * v_lab_basura_discount_pct / 100;
  ELSE
    v_lab_basura_discount := 0;
  END IF;

  -- Calculate total discount (reception-level + lab sample quality)
  v_total_weight_discount := v_violetas_discount + v_humedad_discount + v_moho_discount
                           + v_lab_violetas_discount + v_lab_moho_discount + v_lab_basura_discount;
  ...
  -- Insert discount breakdown for Reception-level Violetas
  ...
  -- Insert discount breakdown for Reception-level Humedad
  ...
  -- Insert discount breakdown for Reception-level Moho
  ...
  -- Insert discount breakdown for Lab Sample Violetas
  IF v_lab_violetas_val IS NOT NULL AND v_lab_violetas_discount > 0 THEN
    INSERT INTO desglose_descuentos (
      recepcion_id,
      parametro,
      umbral,
      ...
      v_lab_violetas_discount_pct,
      v_lab_violetas_val,
      v_lab_violetas_discount_pct,
      v_lab_violetas_discount,
      v_user_id
    );
  END IF;

  -- Insert discount breakdown for Lab Sample Moho
  IF v_lab_moho_val IS NOT NULL AND v_lab_moho_discount > 0 THEN
    INSERT INTO desglose_descuentos (
      recepcion_id,
      parametro,
      umbral,
  ...
  -- Insert discount breakdown for Lab Sample Basura
  IF v_lab_basura_val IS NOT NULL AND v_lab_basura_discount > 0 THEN
    INSERT INTO desglose_descuentos (
      recepcion_id,
      parametro,
      umbral,
      ...
      p_recepcion_id,
      'Basura (Muestra Lab)',
      v_lab_basura_discount_pct,
      v_lab_basura_val,
      v_lab_basura_discount_pct,
      v_lab_basura_discount,
      v_user_id
    );
  END IF;
  ...
  RAISE NOTICE 'Applied quality discounts for reception %: reception=% kg, lab=% kg, lab adj=% kg',
    p_recepcion_id, (v_violetas_discount + v_humedad_discount + v_moho_discount),
    (v_lab_violetas_discount + v_lab_moho_discount + v_lab_basura_discount), v_lab_adjustment;
END;
...
DO $$
BEGIN
  RAISE NOTICE 'Migration 33 completed - trigger now includes lab sample quality discounts';
END $$;
