-- Migration 30: Fix trigger to handle weight_kg fallback
-- Date: 2025-11-01
-- Purpose: Update trigger to use weight_kg when original_weight is 0

-- Update the apply_quality_discounts function to handle weight_kg fallback
...
BEGIN
  ...
  IF v_original_weight = 0 THEN
    SELECT COALESCE(SUM(COALESCE(original_weight, weight_kg)), 0) INTO v_original_weight
  ...
  IF v_original_weight > 0 THEN
    UPDATE reception_details SET
      -- Ensure original_weight is populated (use weight_kg as fallback)
      original_weight = COALESCE(original_weight, weight_kg),
      -- Proportional lab sample adjustment for each detail
      lab_sample_adjustment = (COALESCE(original_weight, weight_kg) / v_original_weight) * v_lab_adjustment,
      -- Quality discount for each detail
      discounted_weight = COALESCE(original_weight, weight_kg) - (COALESCE(original_weight, weight_kg) * v_total_weight_discount / v_original_weight),
...
-- Also update the apply_lab_sample_adjustment function
...
DO $$
BEGIN
  RAISE NOTICE 'Migration 30 completed - trigger now handles weight_kg fallback';
END $$;
