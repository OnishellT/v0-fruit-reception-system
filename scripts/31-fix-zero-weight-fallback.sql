-- Migration 31: Fix trigger to handle zero weights properly
-- Date: 2025-11-01
-- Purpose: Update trigger to use weight_kg when original_weight is NULL or 0

-- Update the apply_lab_sample_adjustment function
...
BEGIN
  ...
  -- Use weight_kg if original_weight is NULL or 0
  IF v_original_weight > 0 THEN
    UPDATE reception_details SET
      -- Ensure original_weight is populated (use weight_kg if NULL or 0)
      original_weight = CASE
        WHEN original_weight IS NULL OR original_weight = 0 THEN weight_kg
        ELSE original_weight
      END,
      lab_sample_adjustment = (CASE
        WHEN original_weight IS NULL OR original_weight = 0 THEN weight_kg
        ELSE original_weight
      END / v_original_weight) * v_lab_adjustment
...
-- Also update apply_quality_discounts
...
DO $$
BEGIN
  RAISE NOTICE 'Migration 31 completed - triggers now handle zero weights correctly';
END $$;
