...
import { createDailyPrice } from "@/lib/actions/cash/prices";
...
export function PriceForm() {
  ...
  // Mock fruit types - will be replaced with real data
  const fruitTypes = [
    { id: "1", name: "Café" },
    { id: "2", name: "Cacao" },
    { id: "3", name: "Miel" },
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await createDailyPrice({
        fruit_type_id: parseInt(formData.fruitTypeId),
      ...
      if (result.success) {
        ...
        toast.error(result.error || "Error al establecer el precio");
      }
    } catch (error) {
