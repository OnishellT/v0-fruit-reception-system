import { pgTable, uuid, varchar, text, boolean, timestamp, date, time, integer, numeric, jsonb, primaryKey, AnyPgColumn, unique } from 'drizzle-orm/pg-core';
...
export const laboratorySamples = pgTable('laboratory_samples', {
  ...
  batchId: uuid('batch_id').references(() => cacaoBatches.id),
...
// Unified quality thresholds table
export const qualityThresholds = pgTable('quality_thresholds', {
  id: uuid('id').primaryKey().defaultRandom(),
  fruitTypeId: uuid('fruit_type_id')
    .references(() => fruitTypes.id, { onDelete: 'cascade' })
    .notNull(),
  ...
  unique: unique().on(table.fruitTypeId, table.metric),
}));
...
export const receptions = pgTable('receptions', {
  ...
  pricingCalculationId: uuid('pricing_calculation_id'),
...
export const fruitTypesRelations = relations(fruitTypes, ({ many }) => ({
  ...
  receptions: many(receptions),
  qualityThresholds: many(qualityThresholds),
}));
...
export const qualityThresholdsRelations = relations(qualityThresholds, ({ one }) => ({
  fruitType: one(fruitTypes, {
    fields: [qualityThresholds.fruitTypeId],
    references: [fruitTypes.id],
  }),
  createdByUser: one(users, {
    fields: [qualityThresholds.createdBy],
    references: [users.id],
  }),
  updatedByUser: one(users, {
    fields: [qualityThresholds.updatedBy],
    references: [users.id],
  }),
}));
