...
export function validateWeightDiscountInputs(
    ...
    // Validate quality metric values are within valid ranges
    const invalidMetrics = [];
    for (const [metric, value] of Object.entries(qualityData)) {
        const numValue = Number(value);
        if (numValue < 0 || numValue > 100) {
            invalidMetrics.push(
                `${metric} debe estar entre 0 y 100 (actual: ${numValue})`
            );
        }
    }

    if (invalidMetrics.length > 0) {
        errors.push(...invalidMetrics);
    }
...
/**
 * Create weight discount breakdown for database storage
 *
