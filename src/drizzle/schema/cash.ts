...
// Cash Reception Details Table (Individual Weighings/Pesadas)
export const cashReceptionDetails = pgTable("cash_reception_details", {
  id: serial("id").primaryKey(),
  receptionId: integer("reception_id").notNull().references(() => cashReceptions.id, { onDelete: 'cascade' }),
  weighingNumber: integer("weighing_number").notNull(),
  containersCount: integer("containers_count").notNull(),
...
export type CashReceptionDetail = typeof cashReceptionDetails.$inferSelect;
export type NewCashReceptionDetail = typeof cashReceptionDetails.$inferInsert;